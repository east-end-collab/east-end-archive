...
const readStream = fs
  ...
  .on("end", () => {
    results.forEach((record) => {
      ...
      let deathDate = formatDate(
        record.Death_Day,
        record.Death_Month,
        record.Death_Year
      );
      let burialDate = formatDate(
        record.Burial_Day,
        record.Burial_Month,
        record.Burial_Year
      );
      ...
      const getDecade = (year) =>
        year ? `${year.slice(0, 3)}0s` : "[unknown]";
    ...
    fs.writeFile(allRecordsDest, resultString, function (err) {
      if (err) {
        ...
        console.log(
          `>>> ${csvSource} converted to json here: ${allRecordsDest}`
        );
      }
    });
    ...
    fs.writeFile(
      fullBioRecordsDest,
      JSON.stringify(fullBioRecords),
      function (err) {
        if (err) {
          console.log(err);
        } else {
          console.log(
            `>>> Full bio records generated here: ${fullBioRecordsDest}`
          );
        }
      }
    );
    ...
    fs.writeFile(
      testRecordsDest,
      JSON.stringify(fullBioRecords.slice(0, 1)),
      function (err) {
        if (err) {
          console.log(err);
        } else {
          console.log(
            `>>> Test bio records generated here: ${testRecordsDest}`
          );
        }
      }
    );
